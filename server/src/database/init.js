...
export async function initDatabase() {
  ...
  setInterval(() => saveDatabase(db), 10000);
...
function createTables(db) {
  ...
  try {
    db.run(`ALTER TABLE races ADD COLUMN difficulty TEXT DEFAULT 'mixed'`);
  } catch (e) {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS race_participants (
      ...
      current_route_point INTEGER DEFAULT 0,
  ...
  // Add custom_route column if it doesn't exist (migration)
  try {
    db.run(`ALTER TABLE race_participants ADD COLUMN custom_route TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    db.run(`ALTER TABLE race_participants ADD COLUMN current_route_point INTEGER DEFAULT 0`);
  } catch (e) {}
  
  // Add boost columns (migration)
  try {
    db.run(`ALTER TABLE race_participants ADD COLUMN boost_energy INTEGER DEFAULT 0`);
  } catch (e) {}
  try {
    db.run(`ALTER TABLE race_participants ADD COLUMN boost_active_until INTEGER DEFAULT 0`);
  } catch (e) {}
  try {
